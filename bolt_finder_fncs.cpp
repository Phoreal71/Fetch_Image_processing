...
//FIX ME!!
const float MIN_X_BOLT = 0.5; //include points starting 0.4m in front of robot 
const float MAX_X_BOLT = 0.8; //include points out to 0.9m in front of robot
const float MIN_Y_BOLT = -1.0; //include points starting -0.5m to left of robot
const float MAX_Y_BOLT = 1.0; //include points up to 0.5m to right of robot
const float MIN_DZ_BOLT = 0.01; //0.01; //box filter from this height above the table top
...
bool ObjectFinder::find_bolts(float table_height, vector<float> &x_centroids_wrt_robot, vector<float> &y_centroids_wrt_robot,
    ...
    box_pt_min << MIN_X_BOLT, MIN_Y_BOLT, table_height + MIN_DZ_BOLT, 0; //from MIN_DZ above table top
    box_pt_max << MAX_X_BOLT, MAX_Y_BOLT, table_height + MAX_DZ_BOLT, 0;
